...
/**
 * Class MaxDiscountController
 *
 * @package App\Controller
 */
class MaxDiscountController extends AbstractController
{
    /**
    ...
    public function list(Request $request, ConfiguratorModelRepository $configuratorModelRepository)
    {
		...
		*/
		...
		$limit = 10;
        $discountList = $this->em->getRepository(TemporaryList::class)->getAllDiscounts('discount', $page, $sort, $dir, $limit, $filter);
        
        $list = $this->prepareDiscountsData($discountList['result'], $configuratorModelRepository);
		$maxPages = ceil($discountList['count'] / $limit);
        ...
        foreach($discountList['result'] as $item) {
            /**
             * @var Discount $item
             */
        ...
        $result = $this->render('discount/list.html.twig',
            [
                'list' => $list,
    ...
    {
        $discount = $this->em->getRepository(TemporaryList::class)->find($id);
 
        $criteria = ['model_slug' => $discount->getModel(),'jato_vehicle_id'=>$discount->getVersion()];
        ...
        return $this->render('discount/details.html.twig',
            [
                'discount' => $discount,
                'car' => $car,
                'options' => $options,
    ...
    private function prepareDiscountsData($discounts, ConfiguratorModelRepository $configuratorModelRepository): array
    {
        ...
        if($discounts) {
            foreach ($discounts as $discount) {
                 ...
                 */                
                $car = $configuratorModelRepository->getFilteredModels(array('model_slug','cabine','jato_vehicle_id', 'power', 'gear', 'fuel'),array('model_slug' => $discount['model'],'jato_vehicle_id' => $discount['version']),array(),'cabine')->fetch();
				$image = $configuratorModelRepository->getImageByModelAndBody($car['model_slug'],$car['cabine']);
